...
class YmlParser:
    ...
    def gen_test_cmds(self, test_files="test_data/", iqtree=None, bin="build", out_file="test_cmds") -> list:
        """
    ...
    def gen_specific_test(self):
        for test in self.data["specific_test"]:
            for key in test:
                if key == "cmd":
                    self.cmds.append(test["cmd"])
                if key == "EXPECTEDLE":
                    self.cmds.append(f"-s {test['aln']}")
                if key == "EXPECTEDGE":
                    continue
                if key == "EXPECTEDGT":
                    continue
...
class CMD:
    def __init__(self, single_aln, part_aln, part_option, option, test_dir, bin, specific_test=None):
        """
